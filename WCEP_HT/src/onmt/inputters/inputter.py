...
def build_dataset(fields, data_type, src_data_iter=None, src_path=None,
    ...
    if data_type == 'text':
        ...
        # add the parsing info here
        parsing_pt_file = '../data/Multi-XScience/parsing/MULTIXSCIENCE_parsing.test.1.pt'
        parsing_pt_file2 = '../data/Multi-XScience/parsing/MULTIXSCIENCE_parsing.test.2.pt'
        parsing_pt_file3 = '../data/Multi-XScience/parsing/MULTIXSCIENCE_parsing.test.3.pt'
        dep_parsing_info = torch.load(parsing_pt_file)
        temp_parsing_info = torch.load(parsing_pt_file2)
        dep_parsing_info += temp_parsing_info
        temp_parsing_info = torch.load(parsing_pt_file3)
        dep_parsing_info += temp_parsing_info

        for i, item in enumerate(dataset):
            item.parsing_info = dep_parsing_info[i]
...
def lazily_load_dataset(corpus_type, opt):
    ...
    def _lazy_dataset_loader(pt_file, corpus_type):
        # pt_file = '../data/newser/OpenNMT-input/newser.train.4.pt'
        ...
        # load dep parsing pt
        parsing_pt_file = pt_file.replace('texts/', 'parsing/') \
            .replace('/newser.', '/MULTIXSCIENCE_parsing.')
        # dep_parsing_info = torch.load('../data/newser/newser_parsing/newser_parsing.valid.0.pt')
        dep_parsing_info = torch.load(parsing_pt_file)
    ...
    if pts:
        for pt in pts:
            # if pt == '../data/newser/OpenNMT-input/newser.train.4.pt':
            #     continue
