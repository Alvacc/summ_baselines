...
def train(args,device_id):
    ...
    def train_iter_fct():
        return data_loader.AbstractiveDataloader(args, load_dataset(args, 'train', shuffle=True, spm=spm), symbols, args.batch_size, device,
                                                 shuffle=True, is_test=False)

    def valid_iter_fct():
        return data_loader.AbstractiveDataloader(args, load_dataset(args, 'valid', shuffle=False, spm=spm), symbols,
                                                   batch_size=15000, device=device, shuffle=False, is_test=False)
    ...
    trainer.train(train_iter_fct, valid_iter_fct, args.train_steps)
...
def wait_and_validate(args, device_id):
    ...
    if (args.test_all):
        ...
        while (True):
            cp_files = sorted(glob.glob(os.path.join(args.model_path)))
            cp_files.sort(key=os.path.getmtime)
            if (cp_files):
                cp = cp_files[-1]
...
def validate(args, device_id, pt, step):
    ...
    valid_iter = data_loader.AbstractiveDataloader(args, load_dataset(args, 'valid', shuffle=False, spm=spm), symbols,
                                                   args.batch_size, device, shuffle=False, is_test=False)

    trainer = build_trainer(args, device_id, model, symbols, vocab_size, None, val=True)
...
def test(args, pt, step):
    ...
    test_iter = data_loader.AbstractiveDataloader(args, load_dataset(args, args.dataset, shuffle=False, spm=spm), symbols,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-data_path', default='../../data/ranked_wiki_b40/WIKI', type=str)
    ...
    parser.add_argument('-result_path', default='../summary/results', type=str)
