...
class Translator(object):
        ...
        with torch.no_grad():
            for batch in data_iter:
                ...
                for trans in translations:
                    pred, gold, src = trans
                    pred_str = ' '.join(pred).replace('<Q>', ' ').replace(r' +', ' ').replace('<unk>', 'UNK').\
                        replace('⁇  ', '').strip()
                    gold_str = ' '.join(gold).replace('<t>', '').replace('</t>', '').replace('<Q>', ' ').\
                        replace(r' +', ' ').strip()
    ...
    def _fast_translate_batch(self,
                              ...
                              min_length=0,
                              n_best=1):
        # TODO1: faster code path for beam_size == 1.

        # TODO1: support these blacklisted features.
        ...
        # Encoder forward.
        src = batch.src
        parsing_info = batch.parsing_info

        src_features, mask_hier = self.model.encoder(src, parsing_info)
