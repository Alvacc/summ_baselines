...
def load_dataset(args, corpus_type, shuffle, spm):
    ...
    def _lazy_dataset_loader(json_file, corpus_type):
        # global last_corpus_type, dataset_precessed
        #
        # if last_corpus_type == corpus_type and len(dataset_precessed) != 0:
        #     last_corpus_type = corpus_type
        #     logger.info('%s dataset exists, number of examples: %d' %
        #                 (corpus_type, len(dataset_precessed)))
        ...
        for item in dataset:
            ...
            for text in item['text']:
                text = ' </t> <t> '.join(text)
                src = spm.encode(text)[0:100]
                data_point['src'].append(src)

            # todo: deal with special tokens
            summary = ' </t> <t> '.join(item['summary'])
            tgt = spm.encode(summary)
            tgt.insert(0, 4)
            tgt.append(5)
            data_point['tgt'] = tgt
            ...
            dataset_precessed.append(data_point)

        # last_corpus_type = corpus_type
        return dataset_precessed

    # Sort the glob output by file name (by increasing indexes).
    jsons = sorted(glob.glob(args.data_path + '.' + corpus_type + '.[0-9]*.jsonl'))
    if jsons:
        if (shuffle):
            random.shuffle(jsons)

        for json in jsons:
            yield _lazy_dataset_loader(json, corpus_type)
    else:
        # Only one inputters.*Dataset, simple!
        json = args.data_path + corpus_type + '.label.jsonl'
        yield _lazy_dataset_loader(json, corpus_type)
